...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    //FINISH ME!
    nav_msgs::Odometry present_state;
    int npts_states = vec_of_states.size();
    bool found_start_pose = false;
    for(int i=0;i<npts_states;i++)
    {
        if(fabs(start_pose.pose.position.x - vec_of_states[i].pose.pose.position.x) < 0.01)
        {
            if(fabs(start_pose.pose.position.y - vec_of_states[i].pose.pose.position.y) < 0.01)
            {
                if(fabs(start_pose.pose.orientation.z - vec_of_states[i].pose.pose.orientation.z) < 0.01)
                {
                    present_state.pose.pose = vec_of_states[i].pose.pose;
                    present_state.twist.twist = vec_of_states[i].twist.twist;
    ...
    if(found_start_pose == false) //just in case, right in the middle of switching modes or other weird situations;
    {
        present_state.pose.pose = start_pose.pose;
        present_state.twist.twist.linear.x = 0.0;
        present_state.twist.twist.linear.y = 0.0;
        present_state.twist.twist.linear.z = 0.0;
        present_state.twist.twist.angular.x = 0.0;
        present_state.twist.twist.angular.y = 0.0;
        present_state.twist.twist.angular.z = 0.0;
        vec_of_states.clear();
        vec_of_states.push_back(present_state);
        return;
    }

    vec_of_states.clear();
    if (fabs(present_state.twist.twist.angular.z)<0.01)//interrupt during a "travel"
    {
        double x_start = present_state.pose.pose.position.x;
        double y_start = present_state.pose.pose.position.y;
        double psi_des = convertPlanarQuat2Psi(present_state.pose.pose.orientation);
        nav_msgs::Odometry des_state;
        des_state.pose.pose = present_state.pose.pose;
        des_state.twist.twist = halt_twist_;
        double t_ramp = present_state.twist.twist.linear.x / accel_max_;
        int npts_ramp = round(t_ramp / dt_);
        ...
        double y_des = y_start;
        double speed_des = present_state.twist.twist.linear.x;
        for (int i = 0; i < npts_ramp; i++) 
        {
            speed_des -= accel_max_*dt_; //Euler one-step integration
            des_state.twist.twist.linear.x = speed_des;
    ...
    else if (fabs(present_state.twist.twist.linear.x)<0.01) //interrupt during a "spin"
    {
        nav_msgs::Odometry des_state;
        des_state.pose.pose = present_state.pose.pose;
        des_state.twist.twist = present_state.twist.twist;
        double psi_start = convertPlanarQuat2Psi(start_pose.pose.orientation);
        double t_ramp = fabs(des_state.twist.twist.angular.z) / alpha_max_;
        int npts_ramp = round(t_ramp / dt_);

        double psi_des = psi_start;
        double omega_des = des_state.twist.twist.angular.z;
        double accel = sgn(omega_des) * alpha_max_;
        for (int i = 0; i < npts_ramp; i++) 
        {
            omega_des -= accel*dt_; //Euler one-step integration
            des_state.twist.twist.angular.z = omega_des;
    ...
    else //interrupt during a "travel" and "spin"
    {
        nav_msgs::Odometry des_state;
        des_state.pose.pose = present_state.pose.pose;
        des_state.twist.twist = present_state.twist.twist;
        //double psi_start = convertPlanarQuat2Psi(start_pose.pose.orientation);
        double t_ramp_spin = fabs(des_state.twist.twist.angular.z) / alpha_max_;
        int npts_ramp_spin = round(t_ramp_spin / dt_);

        double x_start = present_state.pose.pose.position.x;
        double y_start = present_state.pose.pose.position.y;
        double psi_des = convertPlanarQuat2Psi(present_state.pose.pose.orientation);
        double t_ramp_travel = present_state.twist.twist.linear.x / accel_max_;
        int npts_ramp_travel = round(t_ramp_travel / dt_);
        double d_vel = alpha_max_*dt_;
        ...
        double speed_des = present_state.twist.twist.linear.x;
        int npts_ramp;
        if(npts_ramp_travel > npts_ramp_spin)
        {
            npts_ramp = npts_ramp_travel;
        }
        else npts_ramp = npts_ramp_spin;
        for (int i = 0; i < npts_ramp; i++) 
        {
            if(i<npts_ramp_travel)
            {
                speed_des -= accel_max_*dt_; //Euler one-step integration
                des_state.twist.twist.linear.x = speed_des;
            }
